 ...
 *
 * @author Danilo Zelenovic
 */
package coe318.lab1;

public class Resistor {
//Instance (state) variables
//TO DO Add instance variables (Hint: you only need 2!)
    private double resOhm;
    private double resAmp;
...
//Set values of state variables
    this.resOhm = resistance; //Changes this value of the resistance to what is 
                              //given parameter
    }
...
//FIX THIS so that it returns the actual resistance 
        return resOhm;  //Returns value of resistance for use
    }
...
//FIX THIS so that it returns the actual voltage
        return (getCurrent()*getResistance());//Uses formula V=IR in order
    }                                         // to get voltage, as the other
                                              // values are given, voltage isn't
    /**
...
//is between 50 and -50, otherwise print an error message.
        ...
        if (-50>voltage || voltage>50){  //This is to set up the loop desired in        
            System.out.println("Error"); //Comments above - Only sets voltage if
        }                                // within desired range. Calculated 
        else{                            // using V=IR.
            voltage = resAmp*resOhm;
        }
     ...
     * @return the current
     */
    public double getCurrent() { //Returns the value of resAmp (current) for use
//FIX THIS
        return resAmp;
    }
